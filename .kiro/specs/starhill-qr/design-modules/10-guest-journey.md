...
5. **FE.5d — Housekeeping**: HousekeepingView tạo yêu cầu + trạng thái (poll), alreadyOpen. Playwright: request→status, gated.
   - **Hợp đồng verify (HousekeepingContracts.cs)**: `POST /v1/guest/housekeeping{roomId}` → `RequestHousekeepingResponse{ticketId, status('Requested'|'InProgress'|'Done'|'Cancelled'), alreadyOpen}` (idempotent: đã có ticket MỞ → trả ticket đó + alreadyOpen=true); `GET /v1/guest/housekeeping?roomId=` → `GetRoomHousekeepingStatusResult{ticket: HousekeepingTicketView|null}`; `HousekeepingTicketView{ticketId,roomId,status,createdAt,startedAt?,completedAt?}`. Rule-gate + `HousekeepingEnabled` enforce use case (403→/rules, `configuration_unavailable`, `housekeeping_disabled`). POST touch cửa sổ; GET không. UI: null/Done/Cancelled→cho yêu cầu mới; Requested/InProgress→hiện timeline trạng thái + disable nút (đã mở). Poll ~5s cập nhật khi lễ tân đổi trạng thái (Req 6.7).
