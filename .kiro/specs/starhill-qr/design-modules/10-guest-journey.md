...
1. **FE.5a — Core refactor + Rules force-read**: dựng `core/{journeyCore,apiGateway,sessionPersistence}`, **xoá `stores/rules.ts`** (drift), refactor GuestResolveView→set core, RulesView state-machine (§3) + acknowledge, router guards đọc core. Playwright: force-read (Next disabled tới scroll-end+countdown), checkbox→ack→home; mock session_expired→rescan; mock 403→/rules. **Đây là slice sửa drift gốc — ưu tiên.**
...
3. **FE.5c — Chat + realtime**: ChatView (poll + SignalR lazy), gửi plain-text, unread. Playwright (mock hub off→polling path), no-overflow bàn phím.
4. **FE.5d — Housekeeping**: HousekeepingView tạo yêu cầu + trạng thái (poll), alreadyOpen. Playwright: request→status, gated.
