...
### QR-N-088 — Baseline ĐỎ khi rà lại: refactor FE.5a bỏ dở để lại GATEWAY TRÙNG ở guest-web; fix tận gốc + guard chống tái phát (2026-07-27)
- **Bối cảnh**: rà lại toàn bộ trạng thái trước khi tiếp. `dotnet build Platform.slnx -c Release` 0-warning, nhưng `StarHill.ArchitectureTests` **FAIL 1/38**: `FrontendDeliveryGuardTests.QR_AD055_guest_qr_entry_resolves_the_token_and_has_browser_coverage` — `Assert.Contains("resolveGuestToken", entryView)` không tìm thấy.
- **Truy nguyên nhân gốc (không vá triệu chứng)**: FE.5a (QR-AD-057, Candidate B) chuyển guest sang `core/apiGateway.ts` + `core/journeyCore.ts`, `GuestResolveView.vue` nay gọi `apiGateway.resolve(token)`. NHƯNG module cũ `src/api/guestApi.ts` **KHÔNG bị xoá** (khác `rules.ts` đã dọn) → tồn tại:
  1. hàm chết `resolveGuestToken` (không ai gọi) — chính là chuỗi guard đang ghim → guard hoá ra ghim vào code chết, mất hiệu lực gác seam thật;
  2. **class `GuestApiError` THỨ HAI** (`api/guestApi.ts` + `core/apiGateway.ts` mỗi file khai một class cùng tên). Đây là bẫy im lặng: view bắt lỗi bằng `instanceof GuestApiError` của gateway mới; nếu lỗi ném từ class cũ thì `instanceof` TRƯỢT → tụt xuống message generic, mất mã lỗi `rule_ack_required`/`session_expired` (đường force-read/gating hỏng mà không báo).
  Consumer duy nhất còn lại của module cũ là `stores/guestSession.ts` (chỉ import `type GuestResolveResponse`), và store đó chỉ được `HomeView.vue` dùng — `HomeView` **chỉ mount ở route `/demo`** (mockup, QR-N-083); đường guest THẬT là `HomeShell` + JourneyCore. Verify router: `{ path: '/', name: 'home', component: HomeShell }`, `{ path: '/demo', component: HomeView }` → **không có bug runtime trên đường thật**, nhưng là bẫy chờ + guard mất hiệu lực.
- **Đã sửa (gốc)**: (a) `stores/guestSession.ts` lấy `type GuestResolveResponse` từ `core/apiGateway` (MỘT nguồn khai báo) + comment ghi rõ store này DEMO-ONLY; (b) **XOÁ** `src/api/guestApi.ts` (file duy nhất trong `src/api`; e2e grep `/demo|guestApi|resolveGuestToken` = 0 match → blast radius sạch); (c) guard `QR_AD055` ghim SEAM THẬT: `apiGateway.resolve(` trong `GuestResolveView.vue` + `'/v1/guest/resolve'` trong `core/apiGateway.ts` (thay vì file đã xoá); (d) THÊM guard chống tái phát `Guest_web_has_exactly_one_api_gateway_and_one_error_type`: `src/api/guestApi.ts` KHÔNG được tồn tại + đếm `export class GuestApiError` trong toàn `guest-web/src` phải == 1.
- **Bằng chứng**: `dotnet build Platform.slnx -c Release` 0-warning/0-error; `StarHill.ArchitectureTests` **39/39 PASS** (38 cũ + 1 guard mới, đỏ→xanh); `pnpm build:guest` (`vue-tsc --noEmit && vite build`) **EXIT=0** (typecheck xác nhận không còn ai cần module đã xoá); Playwright **65/65 PASS**.
- **Ghi chú môi trường (không phải lỗi code)**: lần chạy `pnpm e2e` đầu fail toàn bộ vì `Executable doesn't exist ... chromium_headless_shell-1228` (binary Playwright chưa tải trên máy này sau khi pnpm resolve lại deps) → `pnpm --filter @starhill/e2e exec playwright install chromium` rồi chạy lại mới xanh. Lần sau gặp e2e fail hàng loạt: kiểm binary TRƯỚC khi nghi code.
- **Drift trong chính chỉ mục anti-drift (đã sửa ở `05`)**: dòng `CURRENT DELTA 2026-07-18` ghi `QR-DV-001..007` + `QR-TO-001..017`, nhưng file thật đã có `QR-DV-008` (mockup guest phải THAY không bổ sung) và `QR-TO-018/019/020`. INV-1..6 không bắt được vì chỉ gác ID liên tục + guard-map của QR-AD, không gác prose range → đã cập nhật tay; nếu lặp lại nên cân nhắc INV mới gác range prose khớp max ID thật.
- **Trạng thái tổng thể sau rà (đã kiểm chứng)**: BE **8/8 module** + `C-GA.5` cascade GuestVisitEnded HOÀN TẤT (QR-N-067, đóng CP9/QR-AD-027). FE: guest journey đủ 4 capability thật (rules/faq/chat/housekeeping, FE.5a→FE.5e) + admin (login/dashboard/rooms/rules/faq). Journal: QR-AD-057, QR-DV-008, QR-TO-020, QR-N-088.
- **NEXT (user chọn, chưa làm)**: (1) i18n `ko/zh` cho UI flow guest (hoàn tất Req 2 mặt UI — hiện chỉ en+vi, ko/zh fallback en); (2) polish thị giác guest đồng bộ token/dark như admin; (3) **FE.5c-ii SignalR realtime** (hạ tầng BE đã sẵn: QR-AD-046 hub `/hubs/chat` + JWT-query + notifier; FE còn polling — QR-TO-020); (4) rà lại `import.meta.env.VITE_STARHILL_MOCK` gating của `buildMockResolve` trong `core/apiGateway.ts` để chắc mock KHÔNG thể bật ở bundle production (chưa verify trong phiên này).

### QR-N-089 — `ARCHITECTURE-REVIEW-2026-07-26.md` là review của DỰ ÁN KHÁC (không áp cho repo này) + audit checklist chuyển giao được → tìm & bịt lỗ mock lọt bundle production (2026-07-27)
- **Kết luận quan trọng nhất (chống drift cho phiên sau)**: file `ARCHITECTURE-REVIEW-2026-07-26.md` ở repo-root **KHÔNG phải review của StarHillGuestApp**. Nó review `vision-platform/` — một hệ Python (hexagonal 4 layer, SHM frame ring, RTSP/ONNX/torch/ZMQ, supervisor process, pytest + import-linter). Bằng chứng: grep toàn repo `shm_frame_ring|SyncLinearExecutor|WriterEpochCoordinator|mask_rtsp|InlineInferenceClient` = **0 match**; không có thư mục `vision-platform/`; repo chỉ có **6 file .py** là script tooling (`validate_ci.py`, `new_module.py`, `validate-release.py`...), không phải ~6.000 LOC Python; review nói `.github/` KHÔNG có workflow nào và baseline `pytest 444 passed` — repo này là .NET 10 + Vue, CI có 2 workflow (`ci.yml` base + `starhill-ci.yml` sản phẩm), test bằng `dotnet test` + Playwright. ⇒ **CD-1..9 / S-1..7 / A-1..6 KHÔNG áp dụng** (không có code tương ứng). ĐỪNG "triển khai" file đó; nếu gặp lại, đọc note này trước.
- **Phần chuyển giao được** (§4 quy trình + §5 nguyên tắc fail-closed) đã audit trên repo này, kết quả ĐO ĐƯỢC:
  - CI: `starhill-ci.yml` đã có job `frontend` (pnpm `--frozen-lockfile` + build 2 SPA + `playwright install --with-deps chromium` + `pnpm e2e`) + `build-test` (build Release 0-warning + full suite) + `docker-image` + `migration-bundle`, mỗi job có `timeout-minutes` → **KHÔNG phải gap**.
  - Lock deps: CPM `Directory.Packages.props` pin CHÍNH XÁC (grep floating `*`/range = 0 match) + `pnpm-lock.yaml` + CI `--frozen-lockfile` → **KHÔNG phải gap**.
  - Secrets không vào log: đã có guard `GuestAccessResolveLogRedactionTests` (QR-AD-025) → **KHÔNG phải gap**.
  - Non-root container: Dockerfile Host `USER $APP_UID` → **KHÔNG phải gap**.
  - Test phụ thuộc OS (mâu thuẫn nặng nhất của review đó): grep `OSPlatform|IsOSPlatform|RuntimeInformation` trong `starhill/**` = 0 → **KHÔNG áp**.
  - Knob vận hành hardcode: chỉ có hằng VALIDATION domain (`MaxNoteLength=500`, `MaxAttempts=5`, `MaxDepth=1000`) — không phải lease/lock-timeout hạ tầng như review → mức độ khác hẳn, **không xử lý đợt này**.
  - **GAP THẬT #1 (đã sửa)**: mock lọt bundle production → QR-AD-058 (dưới đây).
  - **GAP THẬT #2 (CHƯA sửa, có chủ đích)**: Dockerfile Host **thiếu `HEALTHCHECK`** (compose chỉ healthcheck `postgres`/`rabbitmq`; host có `/health/live` + `/health/ready` nhưng không ai probe từ trong container). Không sửa vì **Docker đang tắt** trên máy này → không build/verify được, và image `mcr.microsoft.com/dotnet/aspnet:10.0` chưa xác minh có `curl`/`wget` (nếu không có thì phải `apt-get install` = thêm bề mặt CVE, cần cân nhắc chứ không ship mù). Việc cần làm khi có Docker: probe binary trong image → nếu có `wget`/`curl` thì thêm `HEALTHCHECK` gọi `/health/ready`; nếu không, quyết định giữa (a) cài curl, (b) chỉ dùng probe ngoài (k8s liveness/readiness, reverse proxy) và ghi rõ trade-off.
  - **GAP THẬT #3 (chưa làm)**: không có soak test / SLO latency gate. Đúng là thiếu, nhưng cần user duyệt vì tốn thời gian chạy dài.
- **GAP #1 — đo, chẩn, sửa tận gốc**:
  - **Đo**: marker mock `00000000-0000-0000-0000-000000000101` trong `guest-web/dist/assets/*.js`: `pnpm build` thường → KHÔNG có; `VITE_STARHILL_MOCK=1 pnpm build` → **CÓ**. ⇒ Vite nạp `VITE_*` từ CẢ `process.env`, nên `.env.mock` + `--mode mock` KHÔNG phải hàng rào duy nhất; một biến shell lạc trên máy build/CI là đủ để ship bundle bịa.
  - **Vì sao nguy hiểm hơn "sập"**: mock trả phòng/resort/nội quy/feature-flag GIẢ + `portalWindowExpiresAt: 2099` và KHÔNG gọi backend → khách quét QR thấy giao diện chạy "bình thường" với dữ liệu sai, không log lỗi, không alert. Sai IM LẶNG trên đường guest = mất niềm tin thương mại.
  - **Sửa (QR-AD-058)**: chốt dùng chung `starhill/web/build/assertMockNotBundled.ts`; cả 2 `vite.config.ts` (guest-web + admin-web) chuyển sang hàm `({ command, mode })`, gọi chốt với `loadEnv(mode, cwd, 'VITE_')`; `command === 'build'` + cờ bật (file env HOẶC `process.env`) → **throw** chặn build; `serve` no-op.
  - **Verify 4 tầng**: (1) `pnpm build` (cả 2 SPA) EXIT=0 + bundle guest KHÔNG chứa marker; (2) `VITE_STARHILL_MOCK=1 pnpm build:guest` → EXIT=1 + thông báo "CHẶN BUILD ... biến môi trường shell"; (3) `vite build --mode mock` → EXIT=1 + thông báo chỉ nguồn `.env` của mode "mock"; (4) `pnpm e2e` **65/65 PASS** (dev-server/mock-serve không bị ảnh hưởng). `StarHill.ArchitectureTests` **40/40** (thêm guard `Both_spas_block_the_build_when_mock_mode_is_enabled` khoá cả chốt lẫn việc mỗi SPA thực sự gọi chốt).
- **BÀI HỌC `.gitignore` (bắt tại chỗ, loại lỗi chỉ nổ trên CI/clone sạch)**: chốt ban đầu đặt ở `starhill/web/build/assertMockNotBundled.ts` → `git status` KHÔNG thấy file; `git check-ignore -v` chỉ ra `starhill/.gitignore:4: [Bb]uild/` **ignore cả thư mục**. Nếu commit như vậy: local xanh, nhưng CI/clone sạch thiếu file → `pnpm build` vỡ vì import không phân giải + guard test đỏ. Fix GỐC = đổi chỗ (`web/tooling/`), KHÔNG dùng `git add -f` (bẫy lặp lại cho mọi file sau trong thư mục đó). Quy tắc rút ra: **file mới trong thư mục mới → luôn `git check-ignore -v <path>` trước khi tin `git status` là đủ**; đặc biệt tránh tên `build/`, `dist/`, `obj/`, `bin/`, `out/`. Sau khi đổi: `check-ignore` exit=1 (không ignore), file hiện `?? starhill/web/tooling/...`, đã xoá thư mục `web/build` rỗng; verify lại đủ 4 tầng (build 2 SPA EXIT=0 + bundle sạch marker, build với biến shell EXIT=1, ArchitectureTests 40/40, Playwright 65/65).
- **Ghi chú cho phiên sau**: `ARCHITECTURE-REVIEW-2026-07-26.md` vẫn nằm ở repo-root và **untracked** (tôi không stage). Nếu nó thuộc dự án khác thì nên chuyển ra khỏi repo này để không gây nhiễu; quyết định thuộc user.


### QR-N-090 — Lần đầu chạy cổng runtime với Docker: phát hiện + fix migration drift, phân tích quân sự lỗ hổng cổng, F3 DataProtection defer (2026-07-28)
- **Bối cảnh**: bắt đầu triển khai spec `military-grade-hardening` task 1 (health probe). Khi verify container thật, phát hiện compose Host crash Exited(139) — bug CÓ SẴN, không do thay đổi task 1.
- **Root cause (đo, không đoán)**: base Bedrock thêm feature outbox dead-letter replay (`OutboxReplayOperation`/`OutboxReplayAudit` + index `ix_outbox_dead_letter`, cấu hình `Bedrock.Infrastructure/OutboxInboxModelBuilderExtensions.cs`). Module base Identity đã có migration `AddOutboxReplayAudit`; 3 module starhill CÓ outbox (Identity, GuestAccess, Housekeeping) CHƯA sinh migration tương ứng → `has-pending-model-changes` = true → `ApplyMigrationsOnStartup` ném `PendingModelChangesWarning` → crash. 5 module còn lại sạch. Fix = QR-AD-060.
- **Phân tích quân sự "một mìn nổ ⇒ bãi mìn?"**: chạy FULL BE suite với Docker (lần đầu) = **421/421 pass, 0 fail** (gồm mọi integration Testcontainers áp migration thật: GuestAccess 34, Concierge 44, Rules 42, Faq 37, Housekeeping 28, Rooms 27, ResortConfig 15, Identity 9 + Host.Api.Tests 102 + ArchitectureTests 44 + Html 6 + unit). ⇒ Ở TẦNG MÃ, giả thuyết "rất nhiều bug ẩn" bị bác bỏ bằng bằng chứng. **Vấn đề gốc là QUY TRÌNH: cổng runtime chưa bao giờ được bấm cò** (Docker hay tắt ở phiên trước) — baseline "0-warning + 40/40 arch" chỉ là cổng biên dịch + tĩnh. Test integration đã tồn tại và tốt; chúng chỉ chưa chạy. Nếu chạy, chúng ĐÃ bắt bug (vì cũng gọi MigrateAsync).
- **Lỗ hổng cổng (đã bịt — QR-AD-060)**: không có guard nào kiểm pending-model (`ModuleMigrationHistorySchemaTests` chỉ kiểm history-schema, 4/8 context). Thêm `PendingModelChangesGuardTests` Docker-free phủ 8 context vào CI_PR.
- **GAP#2 HEALTHCHECK (QR-N-089) — nay ĐÓNG**: task 1 = QR-AD-059 (probe bằng `dotnet --healthcheck`, không cài OS package; ghim digest). Verify container thật: healthy 5s, unhealthy ≤40s.
- **F3 — DataProtection chưa cấu hình (defer, LOW cho quy mô mục tiêu)**: grep 2 cây = 0 `AddDataProtection/PersistKeysTo` → Host dùng default (key ở `~/.aspnet/DataProtection-Keys`, mất khi container tái tạo, không mã hoá; log cảnh báo lúc boot). Consumer duy nhất = antiforgery (base `AddAntiforgery`); API là JSON + JWT (admin) + cookie opaque (guest) nên antiforgery gần như trơ. **Đã xác minh: cookie khách `__Host-starhill_guest` mang `IssuedSessionKey` opaque đối chiếu DB → guest session SỐNG SÓT qua restart** (không phụ thuộc DataProtection). ⇒ tác động thực THẤP ở 1-instance; sẽ thành MAJOR nếu chạy đa-instance. Khuyến nghị: đưa "persist + encrypt DataProtection keys" thành 1 requirement hardening riêng — KHÔNG khẩn, chờ user duyệt.
- **Vùng đã rà và SẠCH (bằng chứng phủ định)**: 0 `NotImplementedException/TODO/FIXME/HACK` trong `starhill/src`; 0 empty-catch; 0 `EnsureCreated` lạc; cả 8 context migrate tường minh lúc boot; dòng `fail SELECT __EFMigrationsHistory` trong log boot là hành vi first-run bình thường của EF (dò bảng history chưa tồn tại rồi tạo), không chặn boot.
- **Bài học kỹ thuật**: (a) `dotnet ef migrations add/remove` KHÔNG dùng `--no-build` — assembly cũ khiến `remove` xoá nhầm migration hợp lệ + `add` sinh migration rỗng; đã reset bằng `git checkout` + xoá untracked rồi làm lại có build. (b) `has-pending-model-changes --no-build` cũng đọc assembly cũ → phải rebuild trước khi tin kết quả.
- **Đã commit + push develop** (sync 0/0): `docs(hardening)` design+tasks; `feat(hardening)` task 1 (QR-AD-059); `fix(migrations)` QR-AD-060.
