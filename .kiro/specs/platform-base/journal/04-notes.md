...
### N-078 — Docker bật: toàn suite chạy THẬT 401 pass / 0 skip; fix test-ordering consume e2e (2026-07-12)
- Provenance: user "máy có docker cứ sử dụng"; khởi động Docker Desktop (Server 29.5.2), chạy `dotnet test Platform.slnx` VỚI Docker.
- **Kết quả (empirical, không skip):** 401 test pass / 0 fail / **0 skip** — mọi Testcontainers (Postgres + RabbitMQ) chạy THẬT: Infrastructure.Tests 139 (CP6/CP8/CP15 claim-lease SKIP LOCKED, retention Npgsql, rotation race CP7, integration-event dispatcher), Adapters.RabbitMq.Tests 58 (publish + retry-tier + DLX), Messaging.IntegrationTests 4 (outbox→worker→bus, rotation-emit, consume e2e, transient-retry), Identity.IntegrationTests 3 (migration). Trước đó 18 test này SKIP vì daemon tắt → nay ENFORCED thật.
- **Bug TEST phát hiện + fix tận gốc (KHÔNG phải lỗi production):** `RabbitMqConsumeEndToEndTests.Published_message_...` PRE-DECLARE queue KHÔNG có arg, trong khi consumer (sau rework DLX AD-060/AD-095) declare CÙNG queue CÓ `x-dead-letter-exchange` → RabbitMQ 406 PRECONDITION_FAILED (inequivalent args) → consumer channel chết → không consume → handler không chạy → `Assert.Single` rỗng. Fix: BỎ pre-declare, để consumer TỰ provision topology (đúng chủ sở hữu), start subscriber TRƯỚC → chờ provision (Task.Delay 1s như sibling `Transient_..._retried` đang pass) → mới publish. Gỡ `using RabbitMQ.Client;` thừa (TreatWarningsAsErrors). Bài học: test integration KHÔNG pre-declare topology mà consumer sở hữu — arg mismatch = 406 câm; để consumer provision + chờ ready.
- **Ý nghĩa cho đánh giá review:** phần "production-oriented" của messaging/outbox nay có bằng chứng broker thật (publish/consume/retry/DLX/claim-lease/race). CÒN cho "production-grade" đầy đủ (P1-04 review): test broker-RESTART/recovery, multi-consumer DLQ isolation, shutdown-under-load, `BasicPublishAsync` retry nhận cancellation-token — CHƯA viết (là bước kế nếu đi tiếp Wave 1 P1-04).
- Provenance: `platform/tests/Messaging.IntegrationTests/RabbitMqConsumeEndToEndTests.cs` (fix ordering); full-suite verified in-session 2026-07-12 (Docker 29.5.2).

---

### N-079 — P1-04 resilience messaging: DLQ-isolation + graceful-shutdown-drain có bằng chứng broker thật; broker-restart HOÃN (lý do kỹ thuật) (2026-07-12)
- Provenance: user "tiếp tục cực sâu"; Docker bật → viết + chạy `RabbitMqResilienceTests` (Testcontainers Postgres+RabbitMQ).
- **Cách tiếp cận (verify-then-fix, KHÔNG sửa production đầu cơ):** viết test VALIDATE code recovery/drain/DLX đã có (AD-060/AD-095) dưới nghịch cảnh; chỉ sửa production nếu test lộ defect. Kết quả: **cả 2 test PASS → code đúng, KHÔNG cần sửa** (không thêm cancellation-token đầu cơ vào retry-publish như review gợi ý — chưa có test chứng minh defect; thêm sẽ là suy đoán).
- **Test 1 — DLQ isolation (`Poison_from_one_consumer_lands_only_in_its_own_dead_letter_queue`):** 2 consumer (queue `reslo.a`/`reslo.b`, DLX mặc định `{queue}.dead-letter` — cô lập by construction), poison (event-type lạ → `DeadLettered` → NACK requeue=false → DLX riêng) route theo binding tới từng queue → poison A vào ĐÚNG `reslo.a.dead-letter`, poison B vào `reslo.b.dead-letter`, KHÔNG lẫn (mỗi DLQ đúng 1 message, message thứ 2 = null). Chứng minh DLQ per-consumer không cross-contaminate (review "multi-consumer topology / DLQ per-module").
- **Test 2 — graceful drain (`Graceful_shutdown_completes_in_flight_deliveries_without_loss`):** publish 15, prefetch 20 (tất cả in-flight), handler delay 300ms; chờ `Started==15` (mọi delivery đã vào handler) + khẳng định `Completed<15` (thật sự đang giữa chừng) → `StopAsync` → sau khi trả về: `Completed==15` + inbox mark = 15. Chứng minh `BasicCancel`+`WaitForDeliveries` drain TRỌN in-flight, không cắt ngang/mất (review "shutdown-under-load").
- **⚠️ Broker-restart/recovery HOÃN (lý do kỹ thuật, không bịa "đã test"):** Testcontainers `StopAsync/StartAsync` container RabbitMQ ánh xạ HOST PORT MỚI khi start lại → client auto-recovery (reconnect cùng endpoint cũ) KHÔNG tới được broker mới → không mô phỏng recovery trung thực. Cần harness cổng-cố-định (fixed host port) hoặc `docker pause/unpause` (giữ port) — chưa dựng. Code recovery (`ConnectionShutdownAsync`/`RecoverySucceededAsync`/state `Recovering`→`Consuming`) có + đọc kỹ, nhưng CHƯA có test nghịch cảnh port-stable → ghi rõ là khoảng hở test, không tuyên bố "đã chứng minh recovery".
- **Trạng thái review P1-04:** multi-consumer DLQ isolation ✅ + shutdown-under-load ✅ (có bằng chứng); broker-restart recovery ⏳ (hoãn có lý do). `BasicPublishAsync` retry cancellation-token: KHÔNG đổi (chưa có test chứng minh cần — tránh suy đoán; nếu sau này viết được test broker-stall lộ hang thì fix có bằng chứng).
- **Verify:** build 0-warning; `Messaging.IntegrationTests` 6/6 pass với Docker (thêm 2 resilience). Toàn suite chờ full-run xác nhận.
- Provenance: `platform/tests/Messaging.IntegrationTests/RabbitMqResilienceTests.cs`; verified in-session 2026-07-12 (Docker 29.5.2).


### N-080 — Fix RabbitMqResilienceTests skip-without-Docker (defect N-079) + P1-15 (2026-07-13)
- Máy toann KHÔNG Docker phát hiện defect: `RabbitMqResilienceTests` (thêm ở N-079) để `.Build()` Testcontainers ở FIELD-INITIALIZER (constructor, NGOÀI try/catch) → thiếu Docker ném `DockerUnavailableException` lúc dựng test → test FAIL thay vì SKIP (máy kia có Docker nên không lộ). Vi phạm N-012/N-067 (Build phải trong try để skip). Fix: chuyển `.Build()` vào `InitializeAsync` try + thêm `catch when (!IsContinuousIntegration())` (fail-closed CI, A-07) — mirror `RabbitMqConsumeEndToEndTests`. Sau fix: máy không-Docker → 6 test Messaging.IntegrationTests SKIP (0 fail); `vp all` platform xanh trở lại trên máy không-Docker.
- **P1-15 (AD-100) cùng phiên:** startup fail-fast outbox-producer-không-drainer. Điểm kỹ thuật đáng nhớ: `WebApplicationFactory.ConfigureAppConfiguration` chỉ áp SAU `builder.Build()` → cờ đọc ở COMPOSITION-time (top-level Program.cs) KHÔNG thấy config test-factory (chỉ appsettings placeholder thấy). Vì vậy `RequiredPortsValidator` đọc cờ `Bedrock:Messaging:AllowOutboxWithoutDispatcher` tại StartAsync (POST-build) — không đọc lúc compose. Bài học: cờ cần test-host override PHẢI đọc post-build (hosted service/StartAsync), không đọc top-level.


### N-081 — GROUND-TRUTH re-verification: re-audit 2026-07-12 đã LỖI THỜI nặng (2026-07-13)
- Verify bằng CODE THẬT (không tin ledger/re-audit), phát hiện nhiều finding re-audit `re-audit-architecture-implementation-2026-07-12.md` nay STALE vì commit sau đã đóng:
  - **P1-06** (lease renewal + lease-lost silent): ĐÃ ĐÓNG+GUARD — `EfOutboxDispatcher.TryRenewLeaseAsync` (renew giữa batch) + `RecordLeaseLost(messageId, claimId, phase)` gọi ở 4 phase (renew/published-finalize/dead-letter-finalize/retry-finalize) + `Log.LeaseLost`; guard `OutboxMetricsTests.Ownership_changed_during_publish_records_lease_lost_instead_of_published` + `OutboxDispatcherTests.Expired_lease_is_reclaimed_and_published`.
  - **P1-09** (command/query split): ĐÃ ĐÓNG — `IUseCase.cs` có `IQueryUseCase<TIn,TOut>` (transaction pass-through), `ICommandUseCase<TIn,TOut>` (write trả giá trị, `:ITransactionalUseCase`), `ICommandUseCase<TIn>` (void) — AD-098.
  - **P1-11 (boundary auto-discover)**: ĐÃ ĐÓNG phần project-graph — `DiscoveredProjectBoundaryTests` (AD-099) quét MỌI module/adapter từ disk (layer/cross-module/adapter-tech/slnx-membership) → module/adapter MỚI không thoát guard. `ModuleBoundaryTests`/`AdapterIsolationTests` giữ vai deep-assembly-check (NetArchTest + negative control) trên module/adapter mẫu. CÒN gap NHỎ (đã đóng phiên này QR-AD-018): error-code snapshot chưa phủ catalog module → RoomsErrors lọt.
  - **P1-13** (JWT verify-only ValidateOnStart): ĐÃ ĐÓNG+GUARD — `AddBedrockAuthCore` `.ValidateOnStart()` + validator; test `AuthMechanismTests.Verify_only_host_with_empty_key_ring_fails_during_startup`.
  - **P1-10 (capability coupling)**: ĐÃ ĐÓNG — AddBedrockPersistence KHÔNG tự đăng ký outbox/inbox/refresh (opt-in); guard `MultiModulePersistenceTests.Foundation_does_not_register_schema_dependent_capabilities`. (Bijection đóng phiên này AD-101.)
