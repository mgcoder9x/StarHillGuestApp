...
### AD-100 — [Wave 1] Startup fail-fast: outbox producer PHẢI có dispatcher worker (re-audit P1-15)
- Status: Confirmed
- Date: 2026-07-13
- Decider: user (chốt "fail-fast lúc boot" khi được hỏi 4 phương án) + AI(Kiro) triển khai.
- Provenance/Evidence: đọc 2 Host `Program.cs` (platform + starhill) — messaging gate sau `Bedrock:Messaging:Enabled`; `AddIdentityInfrastructure` gọi `AddBedrockOutbox<IdentityDbContext>` VÔ ĐIỀU KIỆN (producer), nhưng `AddOutboxDispatcherWorker` chỉ khi messaging bật (drainer). Khi tắt: refresh rotation VẪN enqueue `UserTokenRefreshedIntegrationEvent` (đọc `RefreshAccessTokenUseCase`) → outbox phình vô hạn, KHÔNG worker phát → tích lũy IM LẶNG (mất event bảo mật). Đã sửa: mở rộng cơ chế startup-validation sẵn có (AD-011): `StartupValidationOptions` ghi nhận `OutboxProducerContexts` (AddBedrockOutbox) + `OutboxDrainerContexts` (AddOutboxDispatcherWorker) + cờ `OutboxWithoutDispatcherAllowed`; `RequiredPortsValidator.StartAsync` (SAU vòng resolve port — giữ ưu tiên lỗi cũ) chặn boot nếu có producer thiếu drainer VÀ chưa allow-offline. Host thêm nhánh `else` đọc `Bedrock:Messaging:AllowOutboxWithoutDispatcher` (offline tường minh). Guard: `MessagingStartupGuardTests` (4, không Docker): producer-không-drainer→ném; có drainer→ok; allow-offline→ok; không producer→ok. Smoke test 2 cây (SecretInjectingHostFactory) khai cờ offline (boot-offline có ý thức) → vẫn xanh.
- Context: re-audit P1-15 — event commit vào outbox nhưng không bao giờ phát khi messaging tắt; silent accumulation thay vì fail-loud. Rủi ro nghiêm trọng cho sản phẩm thương mại (mất reaction bảo mật, phình DB).
- Decision/Change: producer-không-drainer = fail-fast lúc boot (mặc định), có escape hatch offline TƯỜNG MINH (config/`AllowOutboxWithoutDispatcher`). Khớp per-context (producer `AddBedrockOutbox<TContext>` vs drainer `AddOutboxDispatcherWorker<TContext>` cùng TContext). Đặt check trong `RequiredPortsValidator` SAU vòng resolve required-ports để lỗi JWT/port (vd IJwtTokenService ctor HS256) vẫn ưu tiên (không đổi test cũ).
- Rationale (verifiable): **Root cause:** offline là trạng thái ẩn (mặc định messaging tắt) không ai buộc phải quyết định → misconfig production im lặng. Fix bản chất = biến offline thành QUYẾT ĐỊNH TƯỜNG MINH: hoặc bật messaging (có drainer), hoặc khai allow-offline có ý thức; không khai gì mà có producer = boot nổ. Tận dụng cơ chế startup-validation sẵn có (nhất quán, không thêm mô hình mới). Vì sao đặt trong RequiredPortsValidator (không hosted-service riêng): đảm bảo thứ tự — resolve required-ports (JWT...) chạy trước → giữ nguyên ưu tiên lỗi + test HS256 hiện có.
- Alternatives: (a) không enqueue khi messaging tắt (loại: module phải biết config Host — leaky; mất event lẽ ra phải phát khi bật lại — phá mục đích outbox durable); (b) explicit offline mode + health-degraded (phương án 2 khi hỏi — hợp lệ nhưng user chọn fail-fast cho production, mạnh hơn); (c) giữ nguyên (loại: chính lỗ P1-15). Escape hatch offline giữ được smoke/dev.
...
### AD-102 — [Wave 1] Tách business CorrelationId khỏi W3C trace context (traceparent + tracestate) — re-audit P1-14
- Status: Confirmed
- Date: 2026-07-13
- Decider: user (duyệt cả 3 khuyến nghị Q1/Q2/Q3 của design doc) + AI(Kiro) triển khai theo `design-P1-14-trace-correlation-split.md`.
- Provenance/Evidence: đọc code THẬT — `EfOutboxWriter` set `CorrelationId=Activity.Current?.Id` (traceparent) + VỨT `TraceStateString`; mapper đặt traceparent vào `BasicProperties.CorrelationId` (sai quy ước AMQP); dispatcher `ActivityContext.TryParse(CorrelationId, null)` mất tracestate. Đã sửa (6 increment): (1) envelope `Outgoing/IncomingIntegrationMessage` + `OutboxMessage` thêm `TraceParent`/`TraceState`, `CorrelationId` nay là BUSINESS; (2) EF map 2 cột + migration `AddOutboxTraceContext` (platform + starhill Identity — ADD 2 cột nullable varchar 512/1024, additive) + snapshot; (3) `EfOutboxWriter` capture traceparent+tracestate (giữ tracestate), CorrelationId=null; `EfIntegrationEventDispatcher` `ActivityContext.TryParse(TraceParent, TraceState)` (GIỮ tracestate) + tag business correlation; (4) `RabbitMqMessageMapper` header CHUẨN `traceparent`/`tracestate` + `BasicProperties.CorrelationId`=business; consumer đọc header trace + business; retry-publish bảo toàn header trace; (5) starhill migration mirror; (6) journal. Guard: `IntegrationEventTracePropagationTests` (assert consume span cùng TraceId + ParentSpanId + **tracestate GIỮ NGUYÊN** — điểm mới), `OutboxWriterTests.Enqueue_captures_w3c_trace_context...` (TraceParent+TraceState set, CorrelationId null; cột trace_parent/trace_state tồn tại), `RabbitMqMessageMapperTests.{Trace_context_maps_to_standard_w3c_headers, Trace_headers_absent_when_no_trace_context}`. platform+starhill `vp all`+`vp journal` xanh.
- Context: re-audit P1-14 — trộn semantic (business vs trace), mất tracestate (vendor sampling), không dùng header chuẩn → khó interop OTel/vendor. KHÔNG phải bug chức năng (trace vẫn propagate) mà là đúng-chuẩn/observability cho sản phẩm thương mại.
- Decision/Change: hai khái niệm TÁCH BẠCH — W3C trace (traceparent+tracestate, header chuẩn) vs business CorrelationId (AMQP BasicProperties.CorrelationId, hiện null). Persistence ADDITIVE (2 cột mới, giữ correlation_id — an toàn data, không rename). Dùng `ActivityContext.TryParse(traceParent, traceState)` (BCL, giữ tracestate; không cần propagator/carrier vì đã có 2 string tách rời).
- Rationale (verifiable): **Root cause:** một field/cột/AMQP-property gánh 2 khái niệm + chỉ mang traceparent (mất tracestate) + parse thủ công 1 string. Fix bản chất = tách khái niệm ở MỌI tầng (envelope/persist/wire/dispatch) + dùng cơ chế BCL giữ đủ traceparent+tracestate + header chuẩn W3C (interop). Migration additive tránh rủi ro data (giá trị cũ correlation_id là traceparent, không phải business → không migrate được sạch nếu rename).
- Alternatives: (a) rename correlation_id→trace_parent (loại: data-migration rủi ro); (b) CorrelationId=TraceId (loại: tái-trộn trace vào business — user đã duyệt Q1 giữ null); (c) `DistributedContextPropagator` + carrier giả (loại: thừa khi đã có 2 string tách rời; để dành khi thêm baggage); (d) producer publish span (defer — N1/Q3). 
- Consequences: outbox_message + 2 cột nullable (migration 2 cây). Business CorrelationId hiện null (business-correlation port là FOLLOW-UP — chưa có nguồn). Row outbox cũ (correlation_id=traceparent) sau migrate KHÔNG được đọc làm trace (trace_parent null → fallback ambient) — chỉ ảnh hưởng event tồn đọng lúc migrate (hiếm, không mất data). Publish producer span defer. Consumer/producer cũ-mới tương thích (header mới, bỏ qua được).
- Reversibility: Medium (gỡ cột + revert code; migration Down có sẵn).
- Traceability: re-audit P1-14; supersede cơ chế AD-084 (trace qua CorrelationId) — AD-084 nay đổi mang traceparent+tracestate riêng; design doc `design-P1-14-trace-correlation-split.md`; TO-012/013/014; F34/F21/A-29; user-confirmed Q1/Q2/Q3.

### AD-103 — `ExceptionHandlingMiddleware` map `BadHttpRequestException` → 400 (lỗi binding/parse của framework là lỗi CLIENT, không phải 500)
- Status: Confirmed
- Date: 2026-07-16
- Decider: AI (implementation-time), phát hiện qua probe HTTP runtime của sản phẩm StarHill (browser-substitute)
- Provenance/Evidence: chạy Host thật (docker-compose dev, OpenAPI) rồi probe endpoint. `GET /v1/guest/faq` và `GET /v1/guest/rules` KHÔNG kèm query `roomId` (param required) → ASP.NET ném `Microsoft.AspNetCore.Http.BadHttpRequestException: Required parameter "Guid roomId" was not provided from query string.` (log Host thật, correlation trace). `ExceptionHandlingMiddleware` chỉ có catch `ConcurrencyConflictException`/`UniqueConstraintViolationException` + `catch (Exception)` chung → BadHttpRequestException rơi vào catch chung → `CommonErrors.Unexpected()` → **HTTP 500**. Verify sau fix: cùng request → **400**; `POST /v1/guest/resolve` body 'not-json' (JSON parse fail) cũng → **400** (trước là 500). `Bedrock.Api.Tests` 61/61 pass (thêm `ExceptionHandlingMiddlewareTests.Bad_http_request_becomes_400_not_500`).
- Context: `BadHttpRequestException` là tín hiệu CHUẨN của ASP.NET cho request hỏng ở tầng binding/parse: thiếu/sai required route/query param, body không đọc được, content-length quá lớn... Nó mang sẵn `StatusCode` (thường 400). Middleware không xử riêng → mọi request malformed trả 500 (lỗi server GIẢ) → sai ngữ nghĩa HTTP (4xx là lỗi client) + gây báo động/alert nhầm + log Error nhiễu. Đây là lỗ hổng CHUNG mọi endpoint, không riêng StarHill.
- Decision/Change: thêm `catch (BadHttpRequestException ex)` TRƯỚC `catch (Exception)` chung: nếu response chưa bắt đầu → ghi `CommonErrors.Validation()` (Type=Validation → 400 qua ProblemDetailsWriter); log ở mức **Warning** (`ApiLog.BadRequest`, EventId 1002 — không phải Error server), path qua `PathMasker` (F15), message framework mô tả tên/kiểu param (KHÔNG chứa giá trị người dùng). Không thêm mã lỗi mới (dùng `validation_error` sẵn có → snapshot registry không đổi).
- Rationale (verifiable): **Bản chất (không vá ngọn):** sửa ở MIDDLEWARE (một chỗ) đóng cả LỚP lỗi "malformed request → 500" cho MỌI endpoint hiện tại và tương lai (Rooms/Rules/Faq/Identity...), thay vì vá từng endpoint (làm nullable + validate thủ công từng param — ngọn, dễ sót). Đúng ngữ nghĩa HTTP: binding/parse fail = client error 4xx. Log Warning giảm nhiễu alert. Domain-agnostic → thuộc base (F24/AD-009 tinh thần "vắng/ sai input xử đúng loại").
- Alternatives: (a) làm mỗi param required thành nullable + validate thủ công ở từng endpoint (loại: vá ngọn, lặp lại mọi endpoint, dễ sót endpoint mới); (b) map về 500 giữ nguyên (loại: sai ngữ nghĩa + alert nhầm); (c) trả 400 nhưng không log (loại: mất khả năng chẩn đoán client hỏng — Warning là cân bằng đúng).
- Consequences: mọi request thiếu/sai param binding nay trả 400 ProblemDetails (`validation_error`) thay 500. Endpoint có thể vẫn thêm validation nghiệp vụ riêng (400 với message cụ thể) khi cần — middleware là lưới an toàn tầng cuối.
- Reversibility: High (một catch block + một log method + một test).
- Traceability: F24/AD-009 (phân loại xử lý input theo bản chất), design §3.5 (ExceptionHandlingMiddleware slot #3), ProblemDetailsWriter; phát hiện từ pha QR (StarHill runtime probe); QR-N-051 (cross-ref).

### AD-104 — Verification gate fail-closed: thiếu toolchain/build fail không được tạo kết quả xanh hoặc chạy test trên artifact cũ
- Status: Confirmed
- Date: 2026-07-18
- Decider: AI (current-state audit theo yêu cầu user; lỗi được chứng minh trực tiếp trên máy không có .NET SDK).
- Provenance/Evidence: chạy `platform\scripts\vp.cmd all` trước sửa trên máy không có `dotnet`: PowerShell command-not-found không kết thúc step, `$LASTEXITCODE` còn 0 nên build/test có thể hiện OK — cổng xanh giả. `validate_ci.py` còn phụ thuộc PyYAML ngoài repo và Python Windows cp1252 lỗi khi in tiếng Việt. Sau sửa: `vp build` trả FAILED exit 127; `vp all` = build FAIL + validate-ci OK + test `--no-build` BLOCKED; `python platform/tests/validate_ci.py` PASS trên Python không có PyYAML. Workflow CI nay chạy validator trước setup/build. Validation follow-up N-084 dùng SDK portable .NET 10.0.301: restore sạch, `platform/tools/verify.ps1 all` PASS build 0-warning + validator + full suite 0 failure; `vp journal` PASS; `VerificationGateTests.AD104_verification_scripts_fail_closed_and_ci_runs_the_validator` compile/run local thành công.
- Context: một quality gate chỉ có giá trị khi thiếu compiler/command hoặc build hỏng phải làm pipeline đỏ. Chạy `dotnet test --no-build` sau build fail còn nguy hiểm hơn vì có thể dùng artifact cũ và tạo “test xanh” không thuộc source hiện tại. Validator phụ thuộc package chưa bootstrap cũng tự vô hiệu ở môi trường sạch.
- Decision/Change: (1) `verify.ps1` dùng `$ErrorActionPreference='Stop'`, resolve native command tường minh; thiếu `dotnet`/Python = fail-closed (127). (2) scope `all` chỉ chạy test `--no-build` khi build thành công; nếu không ghi step BLOCKED (125) và tổng kết FAIL. (3) Python chọn `python` hoặc `py -3`. (4) `validate_ci.py` có parser stdlib cho đúng subset invariant mà gate sở hữu, vẫn dùng PyYAML nếu có; output ép UTF-8. (5) `verify.ps1` ép console/output encoding UTF-8 để thông báo fail không bị mojibake trên Windows. (6) `.github/workflows/ci.yml` chạy validator thật. (7) thêm C# architecture guard để entrypoint fail-closed/CI call không biến mất âm thầm.
- Rationale (verifiable): root cause là gate tin `$LASTEXITCODE` trong trường hợp command không tồn tại và không ràng buộc test vào build của chính lượt đó. Fix bản chất = kiểm tiền đề trước khi gọi + dependency graph build→test rõ ràng + validator self-contained. Đây là meta-correctness: mọi guard khác vô nghĩa nếu cổng thực thi chúng có thể xanh giả.
- Alternatives: (a) cài .NET/PyYAML tự động trong local script (loại: che thiếu tiền đề, thêm network/side effect; CI đã sở hữu bootstrap); (b) vẫn chạy `--no-build` để “thu thêm tín hiệu” (loại: artifact stale không chứng minh source hiện tại); (c) chỉ sửa thông báo (loại: exit code vẫn sai); (d) vendored YAML parser/package (loại: invariant workflow nhỏ không cần dependency ngoài).
