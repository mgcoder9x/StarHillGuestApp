...
services.AddOutboxReplay<IdentityDbContext>(IdentityInfrastructureExtensions.PersistenceKey);
services.AddSingleton<IEndpointModule, OutboxReplayEndpointModule>();
services.AddAuthorizationBuilder().AddPolicy(
    OutboxReplayAuthorization.Policy,
    policy => policy
        .RequireAuthenticatedUser()
        .RequireClaim("sub")
        .RequireClaim("permission", OutboxReplayAuthorization.Permission));
...
if (bool.TryParse(configuration["Bedrock:Messaging:Enabled"], out var messagingEnabled) && messagingEnabled)
{
    ...
    {
        o.QueueName = "bedrock-reference.identity";
