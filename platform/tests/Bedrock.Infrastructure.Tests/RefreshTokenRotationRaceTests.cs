...
public sealed class RefreshTokenRotationRaceTests(PostgresFixture fixture)
{
    ...
    public async Task Consume_and_insert_roll_back_together_when_insert_conflicts() // F5 atomic (Postgres thật)
    {
        ...
        await using (var scope = provider.CreateAsyncScope())
        {
            ...
            // consume (ExecuteUpdate trong transaction) + insert trùng hash → UNIQUE violation (Postgres 23505)
            // → EfUnitOfWork dịch sang UniqueConstraintViolationException (AD-069, kernel — không rò EF) → rollback CẢ HAI.
