...
/// (<c>UseNpgsql</c> + <c>MigrationsHistoryTable(..,"housekeeping")</c> — QR-AD-028 + <c>UseSnakeCaseNamingConvention</c>)
/// để migration không drift model. Connection string DUMMY. Mirror RulesDbContextFactory/FaqDbContextFactory.
/// </summary>
public sealed class HousekeepingDbContextFactory : IDesignTimeDbContextFactory<HousekeepingDbContext>
{
    public HousekeepingDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<HousekeepingDbContext>()
            .UseNpgsql(
                "Host=localhost;Database=housekeeping_design_time;Username=postgres;Password=postgres",
                npgsql => npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "housekeeping"))
            .UseSnakeCaseNamingConvention()
            .Options;

        return new HousekeepingDbContext(
