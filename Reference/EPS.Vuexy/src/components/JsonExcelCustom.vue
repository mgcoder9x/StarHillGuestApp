        ...
        methods: {
            async generate() {
                ...
                if (this.type === "html") {
                    ...
                    var a = this.generateXLSX(xlsxBuffer, this.name);
                    return a;
                }
                return this.export(
                    this.jsonToXLS(json),
            ...
            jsonToXLSX(data, worksheet) {
                ...
                const firstHeaderCellStyle = {
                    font: { name: 'Arial', bold: true, sz: 14 }, // Bold and font size
                    alignment: { horizontal: 'center', vertical: 'center', wrapText: true }, // Center alignment
                };
                const headerCellStyle = {
                    font: { name: 'Arial', bold: true, sz: 11 }, // Bold and font size
                    alignment: { horizontal: 'center', vertical: 'center', wrapText: true }, // Center alignment
                };
                const columnHeaderCellStyle = {
                    font: { name: 'Arial', bold: true, sz: 10 },
                    alignment: { wrapText: true }, // <-- thêm wrapText
                };
                ...
                // Add header
                if (this.header) {
                    for (const headerRow of this.header) {
                        const headerValues = typeof headerRow === 'string' ? [headerRow] : headerRow;
                        XLSX.utils.sheet_add_aoa(ws, [headerValues], { origin: rowIndex });
                        for (let col = 0; col < colKeys.length; col++) {
                            const cellAddress = XLSX.utils.encode_cell({ r: rowIndex, c: col });
                            if (ws[cellAddress]) {
                                ws[cellAddress].s = cellAddress == "A1" ? firstHeaderCellStyle : headerCellStyle;
                                if (col > 0) ws[cellAddress].v = ''; // Merge cells horizontally
                            }
                        }
                        //merge header
                        merges.push({ s: { r: rowIndex, c: 0 }, e: { r: rowIndex, c: colKeys.length - 1 } });
                        rowIndex++;
                    }
                    //Merge blank row
                    merges.push({ s: { r: rowIndex, c: 0 }, e: { r: rowIndex, c: colKeys.length - 1 } });
                ...
                // Add column headers
                XLSX.utils.sheet_add_aoa(ws, [colKeys], { origin: rowIndex +1 });
                for (let col = 0; col < colKeys.length; col++) {
                    const cellAddress = XLSX.utils.encode_cell({ r: rowIndex +1, c: col });
                    ws[cellAddress].s = columnHeaderCellStyle;
                }
                rowIndex++;

                // Add data rows
                for (const item of data) {
                    // const rowValues = colKeys.map(key => this.preprocessLongNum(this.valueReformattedForMultilines(item[key])));
                    const rowValues = colKeys.map(key => {
                        let value = item[key];
                        if (value !== "0") {
                            value = this.preprocessLongNum(this.valueReformattedForMultilines(value));
                        }
                        return value;
                    });
                    ...
                    for (let col = 0; col < colKeys.length; col++) {
                        const cellAddress = XLSX.utils.encode_cell({ r: rowIndex +1, c: col });
                        ws[cellAddress].s = generalCellStyle;
                    }
                    rowIndex++;
                }
                ...
                // Add footer
                if (this.footer) {
                    for (const footerRow of this.footer) {
                        const footerValues = typeof footerRow === 'string' ? [footerRow] : footerRow;
                        XLSX.utils.sheet_add_aoa(ws, [footerValues], { origin: rowIndex });
                        for (let col = 0; col < colKeys.length; col++) {
                            const cellAddress = XLSX.utils.encode_cell({ r: rowIndex, c: col });
                            if (ws[cellAddress]) {
                ...
                // Adjust column widths based on maximum length of the values
            ...
            jsonToXLS(data) {
                ...
                if (header) {
                    xlsData += this.parseExtraData(
                        header[0],
                        '<tr><th style="font-size: 24px;" colspan="' + colspan + '">${data}</th></tr>'
                    );
                    ...
                    xlsData += this.parseExtraData(
                        header,
                        '<tr><th style="font-size: 18px;" colspan="' + colspan + '">${data}</th></tr>'
                    );
                }
