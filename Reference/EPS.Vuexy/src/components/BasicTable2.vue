...
export default {
    ...
    mounted() {
        // lstColumns sẽ được khởi tạo bởi watcher với immediate: true
    ...
    watch: {
        columns: {
            handler(newColumns) {
                // Cập nhật lstColumns khi prop columns thay đổi
                this.lstColumns = newColumns.map((col) => ({
                    ...col,
                    visible: col.visible === undefined ? true : col.visible,
                    showColumn: col.visible === undefined ? true : col.visible,
                }))
                
                // Khôi phục cấu hình đã lưu từ localStorage nếu có
                if (this.storageName) {
                    const storage = JSON.parse(localStorage.getItem(this.storageName))
                    if (storage) {
                        // Merge với columns mới, giữ lại cấu hình visible đã lưu
                        this.lstColumns = this.lstColumns.map(col => {
                            const saved = storage.find(s => s.field === col.field)
                            if (saved) {
                                return { ...col, visible: saved.visible, showColumn: saved.showColumn }
                            }
                            return col
                        })
                    }
            ...
            deep: true, // Theo dõi thay đổi sâu trong array
        },
        page() {
    ...
    */
        findActionRowContainer() {
            ...
            const expBtns = btns.filter((b) => matchAny(b, exportLabels))
            ...
            // Case A: Export + Refresh → mép phải
            if (expBtns.length && refBtns.length) {
                const el = lca(expBtns[0], refBtns[0])
                if (el) return { el, mode: 'right' }
            }

            // Case B: chỉ Export → đứng cạnh Export
            if (expBtns.length && !refBtns.length) {
                let el = expBtns[0].parentElement
                while (el && el !== document.body) {
                    const cs = window.getComputedStyle(el)
                    if (cs.display.includes('flex') || cs.display === 'block')
                        break
                    el = el.parentElement
                }
                if (el) return { el, mode: 'next' }
            }

            // Case C: KHÔNG có Export/Refresh → dock mép phải (ưu tiên container của Thêm mới nếu có)
            if (!expBtns.length && !refBtns.length) {
