    ...
    >
        <template #button-content>
            <div class="d-sm-flex d-none user-nav">
                ...
                <span class="user-status">{{ userData.company }}</span>
            </div>
            <b-avatar
                size="40"
                :src="userData.avatar"
        ...
        <b-dropdown-item
            :to="{ name: 'pages-setting-notification' }"
            link-class="d-flex align-items-center"
        >
            <feather-icon size="16" icon="ToolIcon" class="mr-50" />
            <span>{{ $t('Button.Config') }}</span>
        </b-dropdown-item>
        <b-dropdown-item
...
export default {
    ...
    methods: {
        logout() {
            ...
            // Remove userData from localStorage
            localStorage.removeItem(useJwt.jwtConfig.storageUserData)
