...
export default {
  ...
  actions: {
    async loadItems({ commit }, params = {}) {
      ...
      try {
        const res = await eventTypeService.getAll(params)
    ...
    async loadDetail({ commit }, id) {
      commit('SET_LOADING', true)
      try {
        const res = await eventTypeService.getById(id)
    ...
    async createItem({ commit }, payload) {
      ...
      try { return await eventTypeService.create(payload) }
      finally { commit('SET_SAVING', false) }
    },

    async updateItem({ commit }, { id, payload }) {
      commit('SET_SAVING', true)
      try { return await eventTypeService.update(id, payload) }
      finally { commit('SET_SAVING', false) }
    },
