    ...
    <b-card no-body>
        <b-card-body>
            <b-form>
                <b-row>
                    <b-col md="4">
                        <b-form-group label="" label-for="h-compId">
                            <treeselect
                                id="module"
                                ref="moduleTree"
                                v-model="moduleId"
                    ...
                    <b-col md="8">
                        ...
                        >
                            <b-form-checkbox-group v-model="rolePrivileges">
                                <div
                                    v-if="
...
export default {
    ...
    computed: {
        ...
        allModules() {
            ...
            for (var i = 0; i < clonedItems.length; i++) {
                ...
                //     item.name = typeof translated === 'string' ? translated : item.name
                // }
                // for (var j = 0; j < item.children.length; j++) {
                //     if (item.children[j].name) {
                //         const translatedChild = this.$t(item.children[j].name)
                //         item.children[j].name = typeof translatedChild === 'string' ? translatedChild : item.children[j].name
                //     }
                // }
    ...
    methods: {
        ...
        save() {
            ...
            this.$services
                .put(
                    `/roles/${this.$route.params.roleId}/privileges?privileges=${data}`
                )
                .then((response) => {
                    this.$toast({
                        ...
                        position: 'top-right',
                        props: {
                            title: this.$t('System.Role.Message.PrivilegeSuccess'),
                            icon: 'CheckIcon',
                            variant: 'success',
                        },
                    })
                ...
                .catch((error) => {
                    this.$toast({
                        ...
                        position: 'top-right',
                        props: {
                            title: this.$t('System.Role.Message.PrivilegeFailure'),
