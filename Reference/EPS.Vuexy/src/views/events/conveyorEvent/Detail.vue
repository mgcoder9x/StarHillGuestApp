            ...
            <b-card-body>
                <b-form>
                    <b-row>
                        ...
                        <b-col md="6">
                            <b-form-group
                                :label="this.$t('Events.Table.Warning')"
                                label-cols-md="4"
                            >
                                <b-form-input
                                    v-if="!editing"
                                    v-model="event.conveyorType"
                                    :disabled="!editing"
                                />
                            </b-form-group>
                        </b-col>
                    ...
                    <b-row>
                        <b-col class="text-center">
                            <b-button
                                v-if="!editing"
                                v-waves
                                :to="{ path: '/event/conveyorEvent/list' }"
                                type="button"
...
import { ConveyorType } from './warningLevelData'
    ...
    methods: {
        loadEventDetail() {
            this.$services.get(`/event/${this.eventId}`).then((response) => {
                ...
                this.event.conveyorType = this.$t(
                    ConveyorType.find(
                        (x) => x.id == response.data.data.warningLevelId
                    ).label
                )
            })
        },
