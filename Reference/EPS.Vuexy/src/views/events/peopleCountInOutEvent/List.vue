...
<template>
    <div>
        ...
        <b-card title="">
            ...
            <!-- table -->
            <BasicTable
                ref="peopleCountInOutTable"
                :columns="columns"
                data-url="/peopleCountInOutEvent"
                :search-form="searchForm"
                :sort-by="'accessTime'"
                storage-name="peopleCountInOutEventTable"
            >
                <template slot="table-row" slot-scope="props">
                    ...
                    <!-- <span v-else-if="props.column.field === 'gender'">
                        <span v-if="props.row.gender === 1">{{
                            $t('FaceEvent.Detail.Female')
                        }}</span>
                        <span v-else-if="props.row.gender === 0">{{
                    ...
                    <!-- <span v-else-if="props.column.field === 'action'">
                        <div class="center-icon text-nowrap">
                            <b-button
                                ...
                                :to="{
                                    path: `/event/peopleCountInOutEvent/detail/${props.row.eventId}`,
                                }"
                            >
...
export default {
    ...
    data() {
        return {
            ...
            searchForm: {
                ...
                // FilterMode: 2 = PeopleCountInOutEvent (chỉ hiển thị NumberOfPeople != 0)
                filterMode: 2,
            },
            columns: [
                {
                ...
                // { label: 'FaceEvent.Detail.Age', field: 'age' },
                // { label: 'FaceEvent.Detail.Gender', field: 'gender' },
    ...
    created() {
        ...
        const saved = getStorage('peopleCountInOutEventTable')
        if (saved) {
        ...
        // Tính tổng khi component được tạo
        this.$nextTick(() => {
            setTimeout(() => this.calculateTotalPeople(), 1000)
        })
    },
    watch: {
    ...
    methods: {
        ...
        async exportData() {
            ...
            vm.headerExcelDetail = [
                vm.$t('Events.Header.PeopleCountInOut'),
            ...
            const response = await this.$services.get(
                vm.$refs.peopleCountInOutTable.dataUrl + '?' + formData
            )
        ...
        async search() {
            ...
            setStorage('peopleCountInOutEventTable1', this.searchForm, 120)
            this.$refs.peopleCountInOutTable.refresh()
        },
        refresh() {
            clearStorage('peopleCountInOutEventTable')
            this.searchForm = {
