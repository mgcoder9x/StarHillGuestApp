    ...
    <b-card v-if="data" class="card-transaction" no-body>
        ...
        >
            <b-card-title class="mb-0">
                {{ $t('Events.DashBoard.FaceMonitoring') }}</b-card-title
            >
...
export default {
    ...
    methods: {
        async getData() {
            var vm = this
            await this.$services
                .get(`/dashboard/dashBoardFace/${vm.setting.period}`)
                .then((response) => {
        ...
        async getSetting() {
            var vm = this
            await this.$services.get('/dashboard/setting/2').then((rs) => {
                if (rs.data.data) {
                    vm.setting = rs.data.data
                } else {
                    vm.setting.period = 1
                    vm.setting.refreshInterval = 1
                    vm.setting.dashboardType = 2
                }
            })
        },
        saveSettings() {
            this.setting.dashboardType = 2
            this.$services
