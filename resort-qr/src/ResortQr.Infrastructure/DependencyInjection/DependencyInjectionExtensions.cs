...
namespace ResortQr.Infrastructure.DependencyInjection;
...
public static class DependencyInjectionExtensions
{
    ...
    public static IServiceCollection AddResortQrServices(this IServiceCollection services) =>
        services.AddResortQrServices(
            typeof(Application.AssemblyMarker).Assembly,
            typeof(AssemblyMarker).Assembly);

    public static IServiceCollection AddResortQrServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        // một ResortQrDbContext cụ thể mà base KHÔNG cung cấp. Nếu auto-đăng ký, thiếu DbContext sẽ lỗi
        // lúc CHẠY (per-request) thay vì fail-fast lúc khởi động → khó chẩn đoán. Vì vậy persistence được
        // wire TƯỜNG MINH qua AddResortQrPersistence<TContext>() (một đăng ký duy nhất, rõ phụ thuộc).
        services.Scan(scan => scan
            .FromAssemblies(assemblies)
                .AddClasses(c => c.AssignableTo<IScopedService>().NotInNamespaceOf<Persistence.ResortQrDbContext>(), publicOnly: false)
                    .AsImplementedInterfaces()
                    .WithScopedLifetime()
                .AddClasses(c => c.AssignableTo<ISingletonService>().NotInNamespaceOf<Persistence.ResortQrDbContext>(), publicOnly: false)
                    .AsImplementedInterfaces()
                    .WithSingletonLifetime()
                .AddClasses(c => c.AssignableTo<ITransientService>().NotInNamespaceOf<Persistence.ResortQrDbContext>(), publicOnly: false)
                    .AsImplementedInterfaces()
