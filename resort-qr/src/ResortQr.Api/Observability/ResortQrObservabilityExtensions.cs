...
/// </summary>
public static class ResortQrObservabilityExtensions
{
    /// <summary>Serilog JSON + health checks. Serilog không log header mặc định (tránh lộ Authorization/Cookie).</summary>
    public static IServiceCollection AddResortQrObservability(this IServiceCollection services)
    {
    ...
    public static IApplicationBuilder UseResortQrObservability(this IApplicationBuilder app)
    {
    ...
    /// <c>AddResortQrPersistence</c> đăng ký) — Req 14.6/14.7. Nếu app chưa wire persistence thì không có
    /// check nào tag "ready" → trả 200 (sẵn sàng, không có phụ thuộc ngoài để chờ).
    /// </summary>
    public static IEndpointRouteBuilder MapResortQrHealthChecks(this IEndpointRouteBuilder endpoints)
    {
